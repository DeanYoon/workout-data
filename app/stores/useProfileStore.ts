...
export const useProfileStore = create<ProfileStore>((set, get) => {
  ...
  fetchWeightRecords: async () => {
    ...
    // If we have cached data for the same user, use it
    if (isWeightRecordsLoaded && weightRecords.length > 0 && cachedWeightRecordsUserId === userId) {
      set({ isLoading: false });
      return;
    }

    // If already loaded (even if empty) for same user, don't fetch again
    if (isWeightRecordsLoaded && cachedWeightRecordsUserId === userId) {
      set({ isLoading: false });
      return;
    }
  ...
  addWeightRecord: async (weight: number, recordedAt?: string) => {
    try {
      ...
      // Refresh weight records to get updated list
      await get().fetchWeightRecords();
      set({ isSaving: false });
    } catch (error) {
